...
public class Solver
{
    ...
    public List<WorkshopSchedule> getRecommendationsForToday(boolean hardRefresh) throws JsonProcessingException {
        var d1 = new Date(1661241600000l);
        var d2 = new Date();

        int currentWeek = (int)((d2.getTime()-d1.getTime())/604800000) + 1;
        int currentDay = (int)((d2.getTime()-d1.getTime())/86400000) % 7 ;

        if(hardRefresh || week != currentWeek)
        {
            int currentPop = popularityRepository.findByWeek(currentWeek).getPopularity();
            String popResponse = restService.getURLResponse("https://xivapi.com/MJICraftworksPopularity/"+currentPop);
            PopularityJson popJson = objectMapper.readValue(popResponse, new TypeReference<>() {});
            var peaks = peakRepository.findPeaksByDay(currentWeek, currentDay);
    ...
    private Map.Entry<WorkshopSchedule, Integer> getBestBruteForceSchedule(int day,
